...
class UnivariateFeatureAnalyzer():
    def __init__(self, is_continuous=True, feature_is_cont=None, dataset=None, feature_col_name='', suffix="", bins=100, x_axis_limit=None):
    ...
    def _continuous_univariate_visualizations(self):
        ...
        if self.is_continuous == True:
            ...
            # Box Plot and Hist Visualizations
            # print(f"""Visualizations for {self.dataset[self.feature_col_name + self.suffix].name}
            #     """)
            ...
            # Histogram
            plt.subplot(1, 3, 1)
            sns.distplot(self.dataset, bins=self.bins)
