...
class UnivariateFeatureAnalyzer():
    def __init__(self, dataset=None, feature_type=None, suffix="", 
                    feature_col_name='', bins=100, x_axis_limit=None, 
                    feature_col_names=[]):
        ...
        if self.feature_type == 'continuous':
            continuous = ContinuousUnivariateAnalyzer(dataset=self.dataset, feature_col_name=self.feature_col_name, suffix=self.suffix, bins=self.bins, x_axis_limit=self.x_axis_limit)
            continuous.run_eda()

        if self.feature_type == 'discrete':
            discrete = DiscreteUnivariateAnalyzer(dataset=self.dataset, suffix=self.suffix, feature_col_names=self.feature_col_names)
            discrete.run_eda()